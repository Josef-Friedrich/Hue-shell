---
title: Documentation
---

---

# Commands

## Basic commands

* [hue]({{ site.baseurl }}/docs/hue.html)

## Load commands

* [hueload-default]({{ site.baseurl }}/docs/hueload-default.html)
* [hueload-random]({{ site.baseurl }}/docs/hueload-random.html)
* [hueload-scene]({{ site.baseurl }}/docs/hueload-scene.html)

## One color commands

* [huecolor-basic]({{ site.baseurl }}/docs/huecolor-basic.html)
* [huecolor-recipe]({{ site.baseurl }}/docs/huecolor-recipe.html)

## Scene commands

* [huescene-breath]({{ site.baseurl }}/docs/huescene-breath.html)
* [huescene-pendulum]({{ site.baseurl }}/docs/huescene-pendulum.html)
* [huescene-sequence]({{ site.baseurl }}/docs/huescene-sequence.html)

## Service commands

* [hueservice-detect-bridge]({{ site.baseurl }}/docs/hueservice-detect-bridge.html)
* [hueservice-detect-lights]({{ site.baseurl }}/docs/hueservice-detect-lights.html)

## Management commands

* [hue-manager]({{ site.baseurl }}/docs/hue-manager.html)

---

# Integration

`Hue-shell` integrates well into the UNIX ecosystem:

* [triggerhappy]({{ site.baseurl }}/docs/integration-triggerhappy.html)
* [systemd]({{ site.baseurl }}/docs/integration-systemd.html)
* [SysVinit]({{ site.baseurl }}/docs/integration-sysvinit.html)
